...
class Fighter(object):
    ...
    def __init__(self, url):
        ...
        self.pro_range = None    # selector: selecting range to only pro fights, None by default
    ...
    def set_pro_fights(self):
        """
        Sets up range of pro fights for Fighter instance with html code scraper from the site.
        :return: None
        """
    ...
    def set_name(self):
        """
        Collects and sets the name for Fighter instance.
    ...
    def get_validation(self):
        ...
        :return: boolean value or TypeError in case one of the lists was empty (self.validation is False by default)
        """
...
def scrape_all_fighters(filename, filetype='csv'):
    ...
    while fail_counter <= 10:   # scraper will be done after there were 10 non-existing sites (indexes) in a row.
        ...
        if F.set_resource() is None:
            logging.WARNING(f'Loop is about to break, scraper could not open fighter page for index: {fighter_index}.')
            break               # scraper will also break if for some reason it could not get site source.
        F.set_soup()
        if F.set_name() != AttributeError:       # checking if there is existing name for a fighter instance.
            ...
            F.grab_rounds()
            F.grab_time()
            if F.get_validation() != TypeError:  # if there was empty list while validating data, fighter instance will
...
def scrape_ufc_roster(save='no', filetype=None):
    ...
    :param save: string with 'yes' or 'no' depends on if information should be stored in a file. Default is 'no'
...
def scrape_list_of_fighters(fighters_list, filename, filetype='csv'):
    ...
    def scrape_fighter(F):
        """
        Nested function that makes process of scrapping data for fighter instance more convenient.
        :param F: fighter instance object
        :return: None
        """
        F.set_resource()
        F.set_soup()
        if F.set_name() != AttributeError:
            F.set_pro_fights()
            F.grab_result_data()
            F.grab_opponents()
            F.grab_events_date()
